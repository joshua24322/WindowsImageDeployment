...
::echo.
::echo.
rem Build WinPE 3.0 x86 setting
echo Build WinPE 3.0 x86 setting
echo.
for /f "tokens=2 delims={}" %%i in ('bcdedit /store %bcd_path% /create /d "WinPE 3.0 x86" /application osloader') do set guid30x86=%%i
bcdedit /store %bcd_path% /set {bootmgr} default {%guid30x86%} ::set WinPE 3.0 x86 is default item
bcdedit /store %bcd_path% /set {%guid30x86%} device ramdisk=[boot]\sources\winpe_3.0_x86.wim,{ramdiskoptions}
bcdedit /store %bcd_path% /set {%guid30x86%} osdevice ramdisk=[boot]\sources\winpe_3.0_x86.wim,{ramdiskoptions}
bcdedit /store %bcd_path% /set {%guid30x86%} locale en-US
bcdedit /store %bcd_path% /set {%guid30x86%} inherit {bootloadersettings}
bcdedit /store %bcd_path% /set {%guid30x86%} path "\windows\system32\boot\winload.exe"
bcdedit /store %bcd_path% /set {%guid30x86%} systemroot \windows
bcdedit /store %bcd_path% /set {%guid30x86%} detecthal Yes
bcdedit /store %bcd_path% /set {%guid30x86%} winpe Yes
bcdedit /store %bcd_path% /set {%guid30x86%} ems No
echo.
echo.
rem Build WinPE 3.0 x64 setting
echo Build WinPE 3.0 x64 setting
echo.
for /f "tokens=2 delims={}" %%i in ('bcdedit /store %bcd_path% /create /d "WinPE 3.0 x64" /application osloader') do set guid30x64=%%i
bcdedit /store %bcd_path% /set {%guid30x64%} device ramdisk=[boot]\sources\winpe_3.0_amd64.wim,{ramdiskoptions}
bcdedit /store %bcd_path% /set {%guid30x64%} osdevice ramdisk=[boot]\sources\winpe_3.0_amd64.wim,{ramdiskoptions}
bcdedit /store %bcd_path% /set {%guid30x64%} locale en-US
bcdedit /store %bcd_path% /set {%guid30x64%} inherit {bootloadersettings}
bcdedit /store %bcd_path% /set {%guid30x64%} path "\windows\system32\boot\winload.exe"
bcdedit /store %bcd_path% /set {%guid30x64%} systemroot \windows
bcdedit /store %bcd_path% /set {%guid30x64%} detecthal Yes
bcdedit /store %bcd_path% /set {%guid30x64%} winpe Yes
bcdedit /store %bcd_path% /set {%guid30x64%} ems No
echo.
echo.
...
echo.
bcdedit /store %bcd_path% /set {bootmgr} displayorder {%guid30x86%} {%guid30x64%}
::{%guid60x86%} {%guid60x64%}
echo.
echo.
